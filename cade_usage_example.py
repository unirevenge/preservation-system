...
def find_relevant_directives(query: str) -> List[Dict[str, Any]]:
    ...
    for category, directive_list in directives.items():
        for directive in directive_list:
            ...
            directive_text = directive.lower() if isinstance(directive, str) else \
                           str(directive.get("name", "") + " " + \
                               directive.get("description", "")).lower()
            
            if query in directive_text:
                results.append({
                    "category": category,
                    "directive": directive if isinstance(directive, str) else dict(directive)
                })
...
def main():
    ...
    print(f"Total entries in integrated memory: {kb_info.get('integrated_memory_size', 0)}")
