...
class CadeCore:
    ...
    def register_plugin(self, plugin: CadePlugin) -> bool:
        ...
        if plugin.info.name in self.plugins:
            self.logger.warning(f"Plugin {plugin.info.name!r} is already registered")
            return False
        ...
        try:
            if plugin.initialize():
                ...
                self.logger.info(
                    f"Registered plugin: {plugin.info.name!r} v{plugin.info.version}"
                )
                return True
            return False
        except Exception as e:
            self.logger.error(f"Error initializing plugin {plugin.info.name!r}: {e}")
            return False
    ...
    def register_extension(self, extension: CadeExtension) -> bool:
        ...
        if extension.info.name in self.extensions:
            self.logger.warning(
                f"Extension {extension.info.name!r} is already registered"
            )
            return False
        ...
        try:
            if extension.initialize():
                ...
                self.logger.info(
                    f"Registered extension: {extension.info.name!r} v{extension.info.version}"
                )
                return True
            return False
        except Exception as e:
            self.logger.error(
                f"Error initializing extension {extension.info.name!r}: {e}"
            )
            return False
    ...
    def register_directive(self, directive: Directive) -> bool:
        ...
        if directive.name in self.directives:
            self.logger.warning(f"Directive {directive.name!r} is already registered")
            return False

        self.directives[directive.name] = directive
        self.logger.debug(f"Registered directive: {directive.name!r}")
        return True
