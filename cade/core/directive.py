...
class DirectiveResult(BaseModel):
    ...
    success: bool = Field(
        ..., description="Whether the directive executed successfully"
    )
...
class Directive(BaseCadeModel, Generic[T]):
    ...
    requires: List[str] = Field(
        default_factory=list, description="List of required permissions"
    )
    ...
    async def execute(self, **kwargs) -> DirectiveResult:
        ...
        try:
            ...
            if isinstance(result, DirectiveResult):
                return result
            return DirectiveResult.success_result(
